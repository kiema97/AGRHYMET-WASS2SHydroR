test_that("hyd_class_thresholds returns Q1 and Q3 by default", {
  q_hist <- 100:199  # longueur 100, déterministe
  th <- hyd_class_thresholds(q_hist)  # probs = c(0.25, 0.75) par défaut
...
test_that("hyd_class_probs produces valid probabilities that sum to 1", {
  th <- c(t1 = 10, t2 = 20)

  # Cas 1: mu = t1 -> p_below = 0.5
  P1 <- hyd_class_probs(mu = 10, sigma = 2, thresholds = th)
...
test_that("hyd_probs_from_forecast with RMSE returns coherent outputs", {
  ...
  th <- hyd_class_thresholds(q_hist)
...
test_that("hyd_probs_from_forecast accepts vector sigma and custom thresholds", {
  q_hist <- 100:199
  fused_final <- tibble::tibble(
    YYYY = 2000:2003,
...
test_that("hyd_probs_from_forecast applies minimal sigma floor via min_sigma_frac when deriving sigma", {
