...
#' @param years integer vector of years (e.g., 1990:2023)
...
#' @param date_format "YYYY" or "YYYYMMDD"
#' @param mmdd month+day used if date_format = "YYYYMMDD" (default "0701")
#'
...
make_toy_data_by_product <- function(
  ...
  # ---- build YYYY column ----
  if (date_format == "YYYY") {
    YYYY_vec <- as.integer(years)
  } else {
    stopifnot(is.character(mmdd), nchar(mmdd) == 4)
    YYYY_vec <- as.integer(paste0(years, mmdd))
  }
